const global = {
	...
	api: {
		apiKey: '42c7c5cf298f2b3b5781578224f3c904',
...
const createMovieCard = ({ id, title, poster_path, release_date }) => {
	...
	<div class="card-body">
		...
		<p class="card-text">
			<small class="text-muted">Release: ${formatDate(release_date)}</small>
		</p>
	</div>
	`;
...
// Create tv show card
const createTvShowCard = ({ id, name, poster_path, first_air_date }) => {
	const div = document.createElement('div');
	div.classList.add('card');
	div.innerHTML = `
	...
	<div class="card-body">
		...
		<p class="card-text">
			<small class="text-muted">Air Date: ${formatDate(first_air_date)}</small>
		</p>
	</div>
	`;
...
const displayPagination = () => {
	...
	// Disable next button if on last pages
...
const formatDate = date => {
	const dateFormatted = new Date(date.replace(/-/g, '/')).toLocaleDateString();
